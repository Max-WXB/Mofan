...
window = tk.Tk()
window.title('Welcome to Max Python')
window.geometry('450x300')

# welcome image
canvas = tk.Canvas(window, height=200, width=500)
image_file = tk.PhotoImage(file='welcome.gif')
image = canvas.create_image(2,0, anchor='nw', image=image_file)
...
def usr_login():
	...
	if usr_name in usrs_info:
		if usr_pwd == usrs_info[usr_name]:
			...
			tk.messagebox.showwarning(message='Error, your Password is wrong, try again.')
	else:
...
def usr_sign_up():
	...
	btn_comfirm_sign_up = tk.Button(window_sign_up, text='Sign up', command=sign_to_Max_Python)
